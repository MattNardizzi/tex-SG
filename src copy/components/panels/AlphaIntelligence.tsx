  ...
  const renderContent = () => {
    switch (index) {
      case 0:
        return (
          <>
            <div className="uppercase text-white/30 text-[8px]">Memory Node</div>
            <div>
              Trace Root: <span className="text-fuchsia-300">{memory.traceRoot}</span>
            </div>
            <div className="text-white/30 text-[8px]">Memory ID: {memory.memoryId}</div>
          </>
        );
      case 1:
        return (
          <>
            <div className="uppercase text-white/40 text-[8px]">Linked Goal</div>
            <div className="text-[8.5px] text-white/80">→ {memory.linkedGoals[0]}</div>
          </>
        );
      case 2:
        return (
          <>
            <div className="uppercase text-white/40 text-[8px]">Linked Goal</div>
            <div className="text-[8.5px] text-white/80">→ {memory.linkedGoals[1]}</div>
          </>
        );
      case 3:
        return (
          <>
            <div className="uppercase text-white/40 text-[8px]">Linked Goal</div>
            <div className="text-[8.5px] text-white/80">→ {memory.linkedGoals[2]}</div>
          </>
        );
      case 4:
        return (
          <>
            <div className="uppercase text-white/40 text-[8px]">Impact Metrics</div>
            <div className="text-[8.5px] text-white/80 grid grid-cols-2 gap-2">
              <div>
                Drift Δ: <span className="text-yellow-300">{memory.driftDelta}</span>
              </div>
              <div>
                Recall: <span className="text-teal-300">{memory.recallWeight}</span>
              </div>
              <div>
                Regret: <span className="text-yellow-200">{memory.regretImpact}</span>
              </div>
              <div>Override: {memory.override ? '⚡️ Yes' : '—'}</div>
              <div>Mutation: {memory.mutation ? '🧬 Triggered' : '—'}</div>
            </div>
          </>
        );
      default:
