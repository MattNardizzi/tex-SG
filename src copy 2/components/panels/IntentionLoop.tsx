          ...
          <AnimatePresence mode="wait">
            {rotationIndex === 0 && (
              <motion.div
                ...
                className="space-y-1 text-[9px] font-body"
              >
                <div className="uppercase text-white/30 text-[8px] font-body">Strategy</div>
                <div>
                  <span className="text-fuchsia-300 font-mono font-bold">{report.strategy}</span>{' '}
                  on <span className="text-sky-300 font-mono">"{report.future}"</span>
                </div>
              </motion.div>
            )}
            ...
            {rotationIndex === 1 && (
              <motion.div
                ...
                exit={{ opacity: 0, y: -10 }}
                transition={{ duration: 0.6 }}
                className="space-y-2 text-[9px] font-body"
              >
                <div>
                  Emotion:{' '}
                  <span className={`${colorEmotion(report.dominantEmotion)} font-mono`}>
                    {report.dominantEmotion}
                  </span>
                </div>
                <div>
                  Bias: <span className="text-fuchsia-300 font-mono">{report.bias}</span>
                </div>
              </motion.div>
            )}
            ...
            {rotationIndex === 2 && (
              <motion.div
                ...
                exit={{ opacity: 0, y: -10 }}
                transition={{ duration: 0.6 }}
                className="grid grid-cols-2 gap-2 text-[9px] font-body"
              >
                <div>Urgency: <span className="font-mono">{report.urgency}</span></div>
                <div>Coherence: <span className="font-mono">{report.coherence}</span></div>
                <div>Confidence: <span className="font-mono">{report.confidence}</span></div>
                <div>Volatility: <span className="font-mono">{report.volatility}</span></div>
              </motion.div>
            )}
            ...
            {rotationIndex === 3 && (
              <motion.div
                ...
                exit={{ opacity: 0, y: -10 }}
                transition={{ duration: 0.6 }}
                className="space-y-2 text-[8.5px] font-body"
              >
                <div className="uppercase text-white/40 text-[8px] tracking-wide font-body">Swarm Mood</div>
                <div className="grid grid-cols-3 gap-y-1 text-white text-[8px] justify-items-center">
                  {Object.entries(report.swarm).map(([mood, score]) => (
                    <div key={mood} className="font-mono">
                      {mood}: {score}
                    </div>
                  ))}
                </div>
            ...
            {rotationIndex === 4 && (
              <motion.div
                ...
                exit={{ opacity: 0, y: -10 }}
                transition={{ duration: 0.6 }}
                className="text-[8.5px] space-y-2 max-w-xs mx-auto font-body"
              >
            ...
            {rotationIndex === 5 && (
              ...
              >
                <div>
                  Risk Level: <span className="text-white font-mono">{report.riskLevel}</span>
                </div>
                <div>
                  Memory Trace: <span className="text-white font-mono">{report.memoryTrace.toString()}</span>
                </div>
              </motion.div>
            )}
            ...
            {rotationIndex === 6 && (
              <motion.div
                ...
                exit={{ opacity: 0, y: -10 }}
                transition={{ duration: 0.6 }}
                className="space-y-2 text-[9px] text-white/60 font-body"
              >
                <div className="text-white/40 text-[8px] uppercase">Loop Complete</div>
                <div className="text-white font-mono">{new Date(report.timestamp).toLocaleTimeString()}</div>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
