...
export default function ReflexiveCausalityMatrix() {
  ...
  useEffect(() => {
    ...
    const advance = () => {
      ...
      setSlide(current);
      if (current === 0) {
        const data = generateReflexInsight();
        setInsight(data);
        if (parseFloat(data.contradiction) > 0.85) {
          setGlow(true);
  ...
  return (
    <div className={`relative w-full h-full px-6 py-5 rounded-2xl bg-black font-sans overflow-hidden text-[16px] leading-[1.4]
      border-[2px] ${glow ? 'border-[#00f0ff] shadow-[0_0_60px_rgba(0,240,255,0.5)] animate-pulse' : 'border-[#00f0ff22] shadow-[0_0_120px_#000000f0]'} transition-all duration-300`}>
