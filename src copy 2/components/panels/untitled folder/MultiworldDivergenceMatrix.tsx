...
const generateMultiworldInsight = () => {
  ...
  const ghostForks = Array.from({ length: 2 }, () => ({
    ...
    emotion: ['resolve', 'fear', 'curiosity'][Math.floor(Math.random() * 3)],
    lineage: `g${Math.floor(Math.random() * 4)}.${Math.floor(Math.random() * 10)}`,
    coherenceLoss: (Math.random() * 0.3 + 0.1).toFixed(2),
  }));

  return {
    universe: futures[Math.floor(Math.random() * futures.length)],
    divergenceScore: (Math.random() * 1.6 + 0.1).toFixed(2),
    contradictionEntropy: (Math.random() * 0.3).toFixed(3),
    survivalIndex: (Math.random() * 0.3 + 0.7).toFixed(2),
    entangled: Math.random() > 0.55,
    ghostForks,
    simulatedRegret: (Math.random() * 0.9).toFixed(2),
          ...
          >
            {slide === 0 && (
              <>
                ...
                <div className="text-cyan-300 font-body text-[10.5px]">
                  ✳ {insight.universe}
                </div>
              </>
            )}
            ...
            {slide === 1 && (
              <>
                <div>
                  Divergence Score:
                  <span className="text-lime-300 font-mono"> {insight.divergenceScore}</span>
                </div>
                <div>
                  Contradiction Entropy:
                  <span className="text-orange-300 font-mono"> {insight.contradictionEntropy}</span>
                </div>
                <div>
                  Survival Index:
                  <span className="text-emerald-300 font-mono"> {insight.survivalIndex}</span>
                </div>
              </>
            )}
                ...
                <div className="font-mono text-[10px]">
                  {insight.entangled ? (
                    <span className="text-rose-400">
                      ⚡ Entangled with Sovereign Instability
                    </span>
                  ) : (
            ...
            {slide === 4 && (
              <>
                ...
                <div className="text-white/50 text-[9.5px] font-mono">
