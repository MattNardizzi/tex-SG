...
export default function LiveAlphaDecisionPanel() {
  ...
  return (
    <div className="relative w-full h-full px-4 py-3 bg-gradient-to-br from-[#041826] via-black to-[#0c1b32] rounded-2xl border border-cyan-400/40 shadow-[0_0_60px_#00ffff66] text-white font-mono overflow-hidden text-[10px]">
      {/* Glow + Dot Matrix */}
      ...
      <div className="relative z-10 flex flex-col justify-between h-full">
        <div className="text-center text-[11px] uppercase tracking-[0.3em] text-cyan-300 font-bold pb-2">
          Forecast Risk Grid
        </div>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {slide === 0 && (
              <>
                <div>
                  Emotion: <span className="text-blue-300 font-bold">{alpha.emotion}</span>
                </div>
                <div>Confidence: {alpha.confidence}</div>
                <div>Coherence: {alpha.coherence}</div>
            ...
            {slide === 1 && (
              <>
                <div className="text-purple-300">
                  REBALANCE on <span className="text-cyan-300">"{alpha.future}"</span>
                </div>
                <div>
                  Risk Level: <span className="text-rose-400">{alpha.riskLevel}</span>
                </div>
                <div>
                  Regret: <span className="text-yellow-300">{alpha.regret}</span>
                </div>
                <div>Urgency: {alpha.urgency}</div>
              </>
            )}

            {slide === 2 && (
              <>
                <div className="uppercase text-[9px] text-white/40">Asset Allocation</div>
                {assets.slice(0, 2).map(([asset, value]) => (
                  <div key={asset} className="flex justify-between items-center gap-2">
                    <span className="w-20 truncate">
                      {asset.charAt(0).toUpperCase() + asset.slice(1)}:
                    </span>
                    <span className="text-cyan-300 text-[9px]">{asciiBar(value)}</span>
                    <span className="text-right w-8">{(value * 100).toFixed(0)}%</span>
                  </div>
                ))}
              </>
            )}
            ...
            {slide === 4 && (
              <>
                <div className="grid grid-cols-2 gap-x-1 gap-y-[2px] text-[10px]">
                  <div>
                    Mutation Triggered:{' '}
                    {alpha.mutation ? (
            ...
            {slide === 5 && (
              <>
                ...
                <div className="grid grid-cols-3 gap-x-2 gap-y-1 text-[9.5px] text-white leading-tight">
