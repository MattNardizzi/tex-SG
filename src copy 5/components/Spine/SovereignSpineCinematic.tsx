...
// === Fragment Shader: Pulse + Color Crossfade + Taper ===
...
export default function SovereignSpineCinematic() {
  ...
  useFrame(({ clock }) => {
    ...
    const nextColor = new THREE.Color(getEmotion().color)
    ...
    blend.current = Math.min(blend.current + 0.01, 1) // smoother fade
    currentColor.current.lerp(targetColor.current, 0.025) // softer lerp
